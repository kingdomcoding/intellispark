defmodule IntellisparkWeb.StudentLive.FlagDetailSheet do
  @moduledoc """
  Right-docked side-sheet rendered when a flag row is clicked. Shows the
  full description, assignees, state-conditional transition buttons, and
  the flag's paper-trail timeline. Transition calls route through the
  existing Flags code-interface so the notifier fires emails and the
  parent's PubSub subscription refreshes the panel.
  """
      ...
      <section class="px-md py-sm space-y-sm">
        ...
        <div>
          ...
          <ul class="space-y-0.5">
            <li
              :for={a <- active_assignments(@flag)}
              class="text-xs text-abbey"
            >
              {a.user.email}
            </li>
