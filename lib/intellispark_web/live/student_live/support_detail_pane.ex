defmodule IntellisparkWeb.StudentLive.SupportDetailPane do
  @moduledoc """
  Tab-pane variant of `SupportDetailSheet` — same data, no fixed-position
  ...
  def render(assigns) do
    ...
    <div class="bg-white rounded-card shadow-card overflow-hidden">
      ...
      <section class="px-md py-sm space-y-sm">
        ...
        <p :if={@support.description} class="text-sm text-abbey whitespace-pre-line">
          {@support.description}
        </p>
        ...
        <div :if={@decline_form_open?} class="pt-sm border-t border-abbey/10 space-y-xs">
  ...
  def handle_event("submit_decline", %{"reason" => reason}, socket) do
    ...
    case Support.decline_support(support, %{reason: reason}, actor: actor, tenant: tenant) do
      {:ok, _updated} ->
        send(self(), {IntellisparkWeb.StudentLive.SupportDetailSheet, :support_closed})
        {:noreply, socket}
  ...
  defp handle_transition(socket, fun, close_after?) do
    ...
    case fun.(support, actor: actor, tenant: tenant) do
      {:ok, updated} ->
        ...
        send(self(), {IntellisparkWeb.StudentLive.SupportDetailSheet, message})
