...
- **Status `⋯` overflow + `Actions` header** — both wired on the Student Hub. Items: Archive / Transfer / Mark withdrawn / Generate report. Each dispatches a single `student_action_attempt` event that flashes "Coming in Phase 11.5 (student lifecycle)". UI surface ships now; the real state-machine actions land in Phase 11.5.
