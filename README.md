...
This repository now ships **Phase 8** — Insightfull Survey + 13 SEL Dimensions + Indicator Scoring (new Indicators domain with IndicatorScore resource, pure scoring algorithm with threshold constants + property-based tests, submit→Oban pipeline, 13 compile-unrolled Student calculations, Hub Key SEL & Well-Being Indicators panel with per-dimension chips, 13 per-dimension custom-list filters, `mix indicators.recompute` idempotent backfill, 26-item Insightfull seed) — on top of **Phase 7** (Surveys Framework), **Phase 6** (High 5s), **Phase 5** (Actions / Supports / Notes), **Phase 4** (Flag workflow), **Phase 3** (Student Hub), **Phase 2** (Students / Tags / Status / CustomLists), **Phase 1.5** (admin invitations), **Phase 1** (auth + multi-tenancy), and the Phase 0 design-system + tooling baseline. See `../phase-8-insightfull-indicators.md`, `../phase-7-surveys.md`, `../phase-6-high-fives.md`, `../phase-5-actions-supports-notes.md`, `../phase-4-flags.md`, `../phase-3-student-hub.md`, `../phase-2-students-tags-lists.md`, `../phase-1-implementation.md`, and `../phase-1.5-school-invitations.md` for the plans, and ADRs under `docs/architecture/decisions/`.
