...
This repository now ships **Phase 9** — Insights view (full-screen `/insights` LV with 13-dimension sidebar, individual breakdown table, pure-SVG donut chart + legend, NimbleCSV export at `/insights/export.csv`, bulk-toolbar entry from `/students` + "View insights" header link on `/lists/:id`, three cohort modes: `?student_ids=<csv>` / `?list_id=<uuid>` / school-wide) — on top of **Phase 8** (Insightfull + 13 SEL indicators), **Phase 7** (Surveys Framework), **Phase 6** (High 5s), **Phase 5** (Actions / Supports / Notes), **Phase 4** (Flag workflow), **Phase 3** (Student Hub), **Phase 2** (Students / Tags / Status / CustomLists), **Phase 1.5** (admin invitations), **Phase 1** (auth + multi-tenancy), and the Phase 0 design-system + tooling baseline. See `../phase-9-insights-modal.md`, `../phase-8-insightfull-indicators.md`, `../phase-7-surveys.md`, `../phase-6-high-fives.md`, `../phase-5-actions-supports-notes.md`, `../phase-4-flags.md`, `../phase-3-student-hub.md`, `../phase-2-students-tags-lists.md`, `../phase-1-implementation.md`, and `../phase-1.5-school-invitations.md` for the plans, and ADRs under `docs/architecture/decisions/`.
